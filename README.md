...
光靠指数窗口（回看 25 秒）会出问题。实测 Codex 的诊断日志是**每 ~3 秒批写一次**，
SQLite 刷盘再滞后 **5~7 秒**，三段延迟叠起来，对话结束了火还能亮 **20~25 秒**，
看着就像「会话都结束了还在烧 token」。

所以加了一条硬判定：**最后一次流式事件距今超过 5 秒，直接判定熄火**，
读数归零、会话计数也归零，不再留指数尾巴。5 秒是量出来的：刚够盖住一个批写周期，
再短会在生成到一半时闪灭。

- `IDLE_CUTOFF_SECONDS = 5` —— 实时流
- `CC_IDLE_CUTOFF_SECONDS = 30` —— cc-switch 用量库（事后统计，本来就慢，给宽松点）
- 会话计数（`N 个会话在烧`）原来用 45 秒窗口，现在跟实时流共用这条 8 秒判定

实测（真机截图）：生成中 `123 生成 ≈tok/s` 满焰；停手 3 秒后已经是 `0 生成 ≈tok/s` 只剩冷灰。

要说明的是，这并不覆盖全部误报来源：会话结束后 Codex 仍可能发起**真实的额外请求**
（日志里能看到别的 thread 在同一时刻提交，cc-switch 也会跟着落一条小请求），
那部分是货真价实的消耗，摆件照实显示。

## 速率锚在哪条时间轴上

这是实时流最要紧的一处细节。事件自带准确时间戳，但**日志是三秒一批写出来的**，
如果按墙上时钟算指数加权速率，两批之间最新鲜的数据也有 0~3 秒的年龄，
权重把它压到 44%~100% 之间来回摆 —— 读数每三秒掉一次再弹回来。
为了盖住这种假抖动，时间常数只能拉到 3.5 秒，起火和熄火就都变迟钝了。

破法是换个锚点：先扫出**最后一条事件的时刻**，以它为基准算速率（`rate()` 会跳过当前这一秒，
所以锚点取 `最后一条事件 + 1`）。读到多少就是多少，两批之间完全稳定，
时间常数因此能从 3.5 秒砍到 `TAU_SIGNAL = 1.4` 秒。

用恒定 300 事件/秒的合成流复演，把批写造成的伪抖动单独隔离出来：

| 口径 | 稳定段最低 / 最高 | 每三秒的单步回落 |
| --- | --- | --- |
| 按墙上时钟（旧） | 43% | 25% |
| 按最后一条事件（新） | 88% | 0% |

真实日志（40 秒 932 个事件）上跑同样的对比：旧口径 32% + 25% 回落，新口径 90% + 10% 回落。

配套的几个数字也一起收紧了：前端轮询 `POLL_MS = 250`（原 620），火势趋近 `GLIDE_MS = 170`（原 240）。

### 延迟还剩多少

数据源本身的下限是 **0~3 秒**（批写周期），这是 `logs_2.sqlite` 的固有节奏，摆件再怎么调也压不下去；
平均检出延迟约 1.5 秒。所以现在的体感是「起火 1~2 秒、熄灭约 5 秒」。
想再快就得换源 —— 例如直接接在 Codex 与 cc-switch 之间读 SSE 流，那是另一个工程量级的改动。
